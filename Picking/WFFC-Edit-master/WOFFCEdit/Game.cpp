...
void Game::Initialize(HWND window, int width, int height)
{
	...
	//save client area rect for, this is used for picking
...
int Game::MousePicking()
{
	...
	for (int i = 0; i < m_displayList.size(); i++)
	{
		...
		{
			//m_displayList[0].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, pickingVector, pickedDistance) = new DisplayObject Closest;
				
			
			//checking for ray intersection
			if (m_displayList[i].m_model.get()->meshes[y]->boundingBox.Intersects(nearPoint, pickingVector, pickedDistance))
			{
				//xna tutorial for picking
				if (selectedID == -1)
				{
					selectedID = i;
				...
				m_objectSelected = m_displayList[i];
...
void Game::Update(DX::StepTimer const& timer)
{
	...
	if (m_InputCommands.focusObject)
	{
		m_view = m_camera.FocusObject(m_InputCommands, m_objectSelected.m_position);
	}

	else m_view = m_camera.GetViewMatrix();
...
void Game::Render()
{
	...
	Vector3 m_camPosition = m_camera.GetPosition();
	m_sprites->Begin();
	WCHAR   Buffer[256];
	...
	for (int i = 0; i < numRenderObjects; i++)
	{
		...
		if (m_displayList[i].m_wireframe == true)
		{
...
void Game::WireframeOnClick(int objectSelected)
{
	int currObject = objectSelected;

	
	if (currObject != -1)
	{
		m_displayList[currObject].m_wireframe = true;

	}

	if (currObject != prevObject)
	{
		m_displayList[prevObject].m_wireframe = false;
...
void Game::SwitchWireFrameOff(int objectSelected)
{

	m_displayList[objectSelected].m_wireframe = false;
	isWireframeOn = false;
}

void Game::SwitchWireFrameOn(int objectSelected)
{
	m_displayList[objectSelected].m_wireframe = true;
	isWireframeOn = true;
}
