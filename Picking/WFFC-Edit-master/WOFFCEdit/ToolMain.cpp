...
void ToolMain::Tick(MSG *msg)
{
	...
	HWND parentWindow = GetParent(m_toolHandle);
	if (GetActiveWindow() == parentWindow)
	{
		if (m_toolInputCommands.mouse_LB_Down)
		{
			...
			if (m_selectedObject != -1)
			{
				...
				m_d3dRenderer.WireframeOnClick(m_prevSelectedObject);
			}
...
void ToolMain::onActionPaste()
{
	if (copiedObject != NULL)
	{
		...
		m_sceneGraph.back().posX += 1;
		m_sceneGraph.back().ID = m_sceneGraph.size();
		m_d3dRenderer.BuildDisplayList(&m_sceneGraph);
	}
}
...
void ToolMain::onActionTurnWireframe()
{
	if (m_d3dRenderer.isWireframeOn == true)
	{
		m_d3dRenderer.SwitchWireFrameOff(m_prevSelectedObject);
	}

	else
		m_d3dRenderer.SwitchWireFrameOn(m_prevSelectedObject);
	
}

void ToolMain::transformObject(SceneObject object, int id)
{
	m_sceneGraph[id] = object;
	m_d3dRenderer.BuildDisplayList(&m_sceneGraph);
}
...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	case WM_MOUSEMOVE:
		// update the mouse X and Mouse Y which will be sent through to the renderer
	...
	{
		if (!m_toolInputCommands.focusObject)
		{
			m_toolInputCommands.focusObject = true;
		}
		else m_toolInputCommands.focusObject = false;
		m_keyArray[' '] = false;
	}
