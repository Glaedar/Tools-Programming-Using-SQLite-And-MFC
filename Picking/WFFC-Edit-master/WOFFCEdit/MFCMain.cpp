...
BEGIN_MESSAGE_MAP(MFCMain, CWinApp)
	...
	ON_COMMAND(ID_EDIT_COPYING, &MFCMain::CopyFunction)
	ON_COMMAND(ID_EDIT_DELETE, &MFCMain::DeleteFunction)
	ON_COMMAND(ID_EDIT_PASTE, &MFCMain::PasteFunction)
	ON_COMMAND(ID_EDIT_DUPLICATE, &MFCMain::DuplicateFunction)
	ON_COMMAND(ID_BUTTONWIREFRAME, &MFCMain::WireframeSelection)
...
int MFCMain::Run()
{
	...
	while (WM_QUIT != msg.message)
	{
		...
		{	
			...
			{
				SceneObject object;
				m_ObjectInspector.ReturnObjectDataWithID(object);
				m_ToolSystem.transformObject(object, ID);
			}
...
void MFCMain::CopyFunction()
{
	m_ToolSystem.onActionCopy();
}

void MFCMain::DeleteFunction()
{
	m_ToolSystem.onActionDelete();
}

void MFCMain::PasteFunction()
{
	m_ToolSystem.onActionPaste();
}

void MFCMain::DuplicateFunction()
{
	m_ToolSystem.onActionDuplicate();
}

void MFCMain::WireframeSelection()
{
	m_ToolSystem.onActionTurnWireframe();
}

void MFCMain::MenuEditInspector()
{
	m_ObjectInspector.Create(IDD_DIALOG2);	//Start up modeless
